...
from datasetLoaders.LLMsData import open_wikiText
from workloads.LLMsW import train_model_txt_generator, collate_fn_txt_generator, infer_txt_generator, get_project_directory
import torch.quantization
...
def main():
    ...
    output_file_path = f'/home/tyson/Desktop/Contract Work/WELLOILEDMACHINE_LLC/hpc_profiler_library/hpc-profiler/{processed_model_name}/{processed_model_name}_model_output.csv'
    ...
    accelerator = Accelerator(mixed_precision=precision_type, fsdp_plugin=fsdp_plugin)
    ...
    model, peft_config, tokenizer = create_and_prepare_GPTQ_model(
        ...
        output_dir=f"{get_project_directory()}/hpc-profiler/{processed_model_name}_{data}"
    )
    print("Model, PEFT config, and tokenizer are ready and model is quantized.")


    TrainChatData = open_wikiText(collate_fn=collate_fn_txt_generator, tokenizer=tokenizer, Ben_batch_size=Ben_batch_size)
    ...
    model, optim, scheduler, TrainChatData = accelerator.prepare(model, optim, scheduler, TrainChatData)
    monitor_system_utilization(benchmark_time_interval=benchmark_time_interval, model_name=processed_model_name, custom_path="/home/tyson/Desktop/Contract Work/WELLOILEDMACHINE_LLC/hpc_profiler_library/hpc-profiler")
    try:
        ...
        # Training RunTime
        print("Fine-tuning...")
        train_model_txt_generator(model_name, TrainChatData, model, accelerator, optim, scheduler, epoch, custom_path="/home/tyson/Desktop/Contract Work/WELLOILEDMACHINE_LLC/hpc_profiler_library/hpc-profiler")
